...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public ArrayList<TLRPC.MessageEntity> getEntities(CharSequence[] message, boolean allowStrike, boolean parseMarkdown) {
        ...
        final String mono = "`";
        final String pre = "```";
        while (!xyz.nextalone.nagram.NaConfig.INSTANCE.getNewMarkdownParser().Bool() && parseMarkdown && (index = TextUtils.indexOf(message[0], !isPre ? mono : pre, lastIndex)) != -1) {
            if (start == -1) {
        ...
        if (xyz.nextalone.nagram.NaConfig.INSTANCE.getNewMarkdownParser().Bool() && parseMarkdown) {
...
//                }
        ...
        if (parseMarkdown) {
            ...
            if (xyz.nextalone.nagram.NaConfig.INSTANCE.getNewMarkdownParser().Bool()) return entities;

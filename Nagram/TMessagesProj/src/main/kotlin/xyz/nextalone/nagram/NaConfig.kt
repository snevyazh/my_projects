...
object NaConfig {
    ...
    init {
        ...
        // Clean build: Premium-only discovery and composer surfaces stay hidden.
        disableFeaturedStickers.changed(true)
        disablePremiumFavoriteEmojiTags.changed(true)
        disableNonPremiumChannelChatShow.changed(true)
        disableShortcutTagActions.changed(true)
        disablePremiumSendTodo.changed(true)
        disableGifts.changed(true)
        // Clean build: messaging-only interface without Stories or Premium AI summaries.
        disableStories.changed(true)
        summarizeTextButton.setConfigInt(SummarizeTextButtonStatus.DISABLE.value)
        disableAiEditor.changed(true)
        // Clean build: retired Repeat double-tap actions become Disabled.
        if (doubleTapAction.Int() == 6 || doubleTapAction.Int() == 7) {
            doubleTapAction.setConfigInt(0)
        }
    }
}
